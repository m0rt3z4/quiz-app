...
const { statusCodesByRoutes } = require('./statusCodes');

const capitalizeString = (string) => string[0].toUpperCase() + string.slice(1).toLowerCase();

const textIdGeneratorHelper = (string) => {
	let modifiedString = string.split('_');
	if (modifiedString.length > 1) modifiedString.slice(1);
	return modifiedString.map((string, i) => (i === 0 ? string.toLowerCase() : capitalizeString(string))).join('');
};
...
const generalStatusMessagesGenerator = (statusCodes) => {
	...
	for (const statusCode of Object.entries(statusCodes)) {
		if (Array.isArray(statusCode)) {
			const statusCodesByRoutestandardRegex = RegExp(/(?=^[A-Z])^((?!(CLASS)|(MESSAGE)|(NAME)|([a-z])).)*$/);
...
const statusMessages = statusMessagesGenerator({ ...statusCodesByRoutes, ...generalStatusMessagesGenerator(httpStatus) });
